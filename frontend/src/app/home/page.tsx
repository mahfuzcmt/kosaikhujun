...
import { butcherApi, locationApi, resolveImageUrl, District, Thana, ButcherDto } from '@/lib/api';
...
export default function HomePage() {
  ...
  const handleDistrictChange = async (districtId: number) => {
    ...
    searchButchers(districtId, null, searchQuery, true);
  };
  ...
  const handleThanaChange = (thanaId: number) => {
    ...
    searchButchers(selectedDistrict, thanaId, searchQuery, true);
  };

  // Debounce search query: refire search 350ms after user stops typing
  useEffect(() => {
    if (selectedDistrict === null) return;
    const t = setTimeout(() => {
      searchButchers(selectedDistrict, selectedThana, searchQuery, true);
    }, 350);
    return () => clearTimeout(t);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchQuery]);

  const searchButchers = async (districtId: number | null, thanaId: number | null, q: string, reset: boolean = true) => {
    if (reset) {
      setLoading(true);
      setPage(0);
      setHasMore(true);
  ...
  const loadMore = () => {
    if (!loadingMore && hasMore) {
      searchButchers(selectedDistrict, selectedThana, searchQuery, false);
    }
  };
      ...
      <div className="bg-primary text-white p-4 sticky top-0 z-10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">কসাই লাগবে</h1>
          </div>
          <button
            onClick={logout}
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <MagnifyingGlassIcon className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-white/70 pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="কসাইয়ের নাম দিয়ে খুঁজুন"
              className="w-full py-3 pl-10 pr-4 rounded-lg bg-white/10 text-white border border-white/20 outline-none placeholder-white/60"
            />
          </div>
        </div>
      </div>
...
// Generate initials from name (e.g., "মোঃ রহিম" -> "মো")
      ...
      <div className="flex gap-4 cursor-pointer" onClick={onClick}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <h3 className="font-bold text-lg sm:text-xl text-gray-900 truncate">{butcher.name || 'কসাই'}</h3>
          ...
          <div className="mt-1.5 text-base font-medium space-y-1">
            {butcher.cowPrice && (

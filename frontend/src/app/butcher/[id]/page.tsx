...
import { butcherApi, api, resolveImageUrl, ButcherDto } from '@/lib/api';
...
import { HomeIcon as HomeIconSolid, HeartIcon as HeartIconSolid, GiftIcon as GiftIconSolid, UserIcon as UserIconSolid } from '@heroicons/react/24/solid';
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="card">
          <div className="flex items-center gap-4 mb-4">
            <div className={`w-20 h-20 rounded-full flex items-center justify-center overflow-hidden ${butcher.photoUrl ? 'bg-gray-100' : getAvatarColor(butcher.name)}`}>
              {butcher.photoUrl ? (
                <img src={resolveImageUrl(butcher.photoUrl)} alt={butcher.name || ''} className="w-full h-full object-cover rounded-full" />
              ) : (
                <span className="text-2xl font-bold text-white">{getInitials(butcher.name)}</span>
              )}
            </div>
            <div>
        ...
        {availability.length > 0 && (
          <div className="card">
            <h3 className="font-semibold text-gray-800 mb-3">📅 সময়সূচি</h3>
            <div className="flex flex-wrap gap-2">
              {availability.map((a) => {
                ...
                const statusLabel = a.status === 'AVAILABLE' ? 'কাজের জন্য প্রস্তুত'
                  : a.status === 'BOOKED' ? 'বুকিং সম্পন্ন'
                  : 'আজ ফাঁকা নেই';
                return (
